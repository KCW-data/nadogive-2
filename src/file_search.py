...
def make_client(api_key: str):
    if not api_key.strip():
        raise FileSearchError("GEMINI_API_KEY가 없습니다. Streamlit Secrets를 확인하세요.")
    try:
...
def extract_interaction(interaction: Any) -> tuple[str, list[Citation]]:
    texts: list[str] = []
    ...
    for step in getattr(interaction, "steps", []) or []:
        ...
        for block in getattr(step, "content", []) or []:
            ...
            for annotation in getattr(block, "annotations", []) or []:
                ...
                citations.append(
                    Citation(
                        file_name=getattr(annotation, "file_name", "알 수 없는 파일"),
                        page_number=getattr(annotation, "page_number", None),
                        source=getattr(annotation, "source", None),
                        custom_metadata=_metadata_to_dict(getattr(annotation, "custom_metadata", None)),
                    )
                )
    answer = "\n\n".join(texts).strip() or str(getattr(interaction, "output_text", "")).strip()
...
def query(
    ...
    answer, citations = extract_interaction(interaction)
